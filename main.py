...
class MyThread(QThread):
    ...
    def __init__(self, camera_index=0, width=screenSize[0], height=screenSize[1]):
        super().__init__()
    ...
    def process_frame(self, frame):
        ...
        if results.pose_landmarks:
            ...
            if left_shoulder.visibility > 0.5 and right_shoulder.visibility > 0.5:
                ...
                self.print_angles(left_arm_angle, right_arm_angle)
    ...
    def calculate_angle(self, shoulder, elbow, wrist):
        ...
        if angle_degrees < 0:
            angle_degrees += 360
        return str(int(angle_degrees))

    def print_angles(self, left_angle, right_angle):
        # 각도를 출력
        message = f"Left Arm Angle: {left_angle.zfill(3)}, Right Arm Angle: {right_angle.zfill(3)}"
        print(message)
...
class MainApp(QtWidgets.QMainWindow):
    ...
    def open_camera(self):
        ...
        self.camera_thread = MyThread(camera_index, width=screenSize[0], height=screenSize[1])
